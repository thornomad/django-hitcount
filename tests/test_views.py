...
class HitCountTestBase(TestCase):
    ...
    def setUp(self):
        ...
        self.request_post = self.factory.post('/', {'hitcountPK': self.
            hit_count.pk}, REMOTE_ADDR='127.0.0.1', HTTP_USER_AGENT=
            'my_clever_agent', HTTP_X_REQUESTED_WITH='XMLHttpRequest')
        self.request_get = self.factory.get('/', REMOTE_ADDR='127.0.0.1',
            HTTP_USER_AGENT='my_clever_agent', HTTP_X_REQUESTED_WITH=
            'XMLHttpRequest')
...
class UpdateHitCountTests(HitCountTestBase):
    ...
    def test_anonymous_user_hit_counted_after_filter_active(self):
        ...
        with mock.patch('django.utils.timezone.now') as mock_now:
            ...
            response = HitCountMixin.hit_count(self.request_post, self.
                hit_count)
        self.assertTrue(response.hit_counted)
        self.assertEqual(response.hit_message, 'Hit counted: session key')
        response = HitCountMixin.hit_count(self.request_post, self.hit_count)
    ...
    def test_registered_user_hit(self):
        self.request_post.user = User.objects.create_user('john',
    ...
    def test_hits_per_ip_limit(self):
        ...
        for x in range(3):
            ...
            responses.append(HitCountMixin.hit_count(self.request_post,
                self.hit_count))
...
class UpdateHitCountJSONTests(HitCountTestBase):
    ...
    def test_require_post_only(self):
        non_ajax_request = self.factory.get('/', HTTP_X_REQUESTED_WITH=
            'XMLHttpRequest')
    ...
    def test_count_hit_invalid_hitcount_pk(self):
        wrong_pk_request = self.factory.post('/', {'hitcountPK': 15},
            REMOTE_ADDR='127.0.0.1', HTTP_USER_AGENT='my_clever_agent',
...
class UpdateHitCountView(HitCountTestBase):
    ...
    def test_count_hit(self):
        ...
        self.assertEqual(response.context_data['hitcount']['pk'], self.
            hit_count.pk)
